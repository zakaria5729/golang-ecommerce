...
func (r *UserRepository) getUserPasswordByID(id uint) (string, error) {
	...
	err := r.db.Model(&User{}).Where(c.FieldID+" = ?", id).Select(c.UserPassword).Take(&user).Error
	if err != nil {
...
func (r *UserRepository) UpdateUserRole(userID uint, roleID uint) error {
	...
	if err != nil {
		l.Logger.Error("❌ Failed to update user role", "method", "UpdateUserRole", "error", err, "userID", userID, "roleID", roleID)
	}
	return err
}
