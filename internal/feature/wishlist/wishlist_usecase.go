...
func (uc *WishlistUseCase) GetAllWishlistsPaginated(userID uint, includeStr string, pageStr string, pageSizeStr string, productIDFilter string, sortBy, sortOrder string) (*models.PaginatedResponse, error) {
	page, pageSize := utils.ParsePagination(pageStr, pageSizeStr)

	include := utils.ParseCommaSeparatedString(includeStr)
	productID, _ := utils.ParseUint(productIDFilter)

	wishlists, total, err := uc.repo.GetAllWishlistsPaginated(userID, include, page, pageSize, productID, sortBy, sortOrder)
	if err != nil {
	...
	var wishlistPtrs []*Wishlist
	for i := range wishlists {
		wishlistPtrs = append(wishlistPtrs, &wishlists[i])
	}

	return utils.BuildPaginatedResponse(wishlistPtrs, total, page, pageSize), nil
}
...
func (uc *WishlistUseCase) CreateWishlist(userID uint, productIDStr string) (*Wishlist, error) {
	...
	if *productID == 0 {
		logger.Logger.Error("Invalid product ID", "method", "CreateWishlist", "productID", *productID)
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to check existing wishlist: %w", err)
	}

	if exists {
		logger.Logger.Error("Product already in wishlist", "method", "CreateWishlist", "userID", userID, "productID", *productID)
		return nil, errors.New("product is already in wishlist")
	}
	...
	err = uc.repo.CreateWishlist(wishlist)
	if err != nil {
...
func (uc *WishlistUseCase) DeleteWishlistByProduct(userID uint, productIDStr string) error {
	productID, err := utils.ParseUint(productIDStr)
	if err != nil {
	...
	if *productID == 0 {
		logger.Logger.Error("Invalid product ID", "method", "DeleteWishlistByProduct", "productID", *productID)
...
func (uc *WishlistUseCase) ClearUserWishlist(userID uint) error {
	if userID == 0 {
		logger.Logger.Error("Invalid user ID", "method", "ClearUserWishlist", "userID", userID)
