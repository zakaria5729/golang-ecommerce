...
// **REQUIRED
func (r *WishlistRepository) GetAllWishlistsPaginated(userID *uint, productID *uint, include []string, page, pageSize int, sortBy, sortOrder string) ([]Wishlist, int, error) {
...
// 	if err := query.Where(c.FieldID+" = ?", id).First(&wishlist).Error; err != nil {
...
// 	result := r.db.Where(c.FieldID+" = ? AND "+c.WishlistUserID+" = ?", id, userID).Delete(&Wishlist{})
// 	if result.Error != nil {
...
// 	selectFields := r.getSelectableFields(include)
// 	query := r.db.Select(strings.Join(selectFields, ", "))
// 	query = query.Where(c.WishlistUserID+" = ? AND "+c.WishlistProductID+" = ?", userID, productID)
